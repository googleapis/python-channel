...
from .base import CloudChannelReportsServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import CloudChannelReportsServiceGrpcTransport


class CloudChannelReportsServiceGrpcAsyncIOTransport(CloudChannelReportsServiceTransport):
    ...
    def run_report_job(self) -> Callable[
        ...
        r"""Return a callable for the run report job method over gRPC.
        ...
        if 'run_report_job' not in self._stubs:
            self._stubs['run_report_job'] = self.grpc_channel.unary_unary(
                '/google.cloud.channel.v1.CloudChannelReportsService/RunReportJob',
                request_serializer=reports_service.RunReportJobRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs['run_report_job']
    ...
    def fetch_report_results(self) -> Callable[
        ...
        r"""Return a callable for the fetch report results method over gRPC.
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'fetch_report_results' not in self._stubs:
            self._stubs['fetch_report_results'] = self.grpc_channel.unary_unary(
                '/google.cloud.channel.v1.CloudChannelReportsService/FetchReportResults',
                request_serializer=reports_service.FetchReportResultsRequest.serialize,
                response_deserializer=reports_service.FetchReportResultsResponse.deserialize,
            )
    ...
    def list_reports(self) -> Callable[
        ...
        if 'list_reports' not in self._stubs:
            self._stubs['list_reports'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=reports_service.ListReportsRequest.serialize,
                response_deserializer=reports_service.ListReportsResponse.deserialize,
            )
...
__all__ = (
    'CloudChannelReportsServiceGrpcAsyncIOTransport',
)
